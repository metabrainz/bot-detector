...
func parseDurations() error {
    ...
    if !dryRun {
        ...
        if err != nil { return fmt.Errorf("invalid poll-interval format: %w", err) }
        cleanupInterval, err = time.ParseDuration(cleanupIntervalStr)
        if err != nil { return fmt.Errorf("invalid cleanup-interval format: %w", err) }
        idleTimeout, err = time.ParseDuration(idleTimeoutStr)
        if err != nil { return fmt.Errorf("invalid idle-timeout format: %w", err) }
    }
    return nil
}
...
func loadChainsFromYAML() ([]BehavioralChain, error) {
	...
	if err != nil { return nil, fmt.Errorf("failed to read YAML file %s: %w", yamlFilePath, err) }

	var config ChainConfig
	if err := yaml.Unmarshal(data, &config); err != nil { return nil, fmt.Errorf("failed to unmarshal YAML: %w", err) }
    ...
    for _, yamlChain := range config.Chains {
		...
		if yamlChain.BlockDuration != "" {
			...
			if err != nil { return nil, fmt.Errorf("chain '%s': failed to parse block_duration '%s': %w", yamlChain.Name, yamlChain.BlockDuration, err) }
		}
        ...
        for _, yamlStep := range yamlChain.Steps {
            ...
            if yamlStep.MaxDelaySeconds != "" {
                ...
                if err != nil { return nil, fmt.Errorf("chain '%s', step %d: failed to parse max_delay '%s': %w", yamlChain.Name, yamlStep.Order, yamlStep.MaxDelaySeconds, err) }
            }
            ...
            for field, regexStr := range yamlStep.FieldMatches {
                ...
                if err != nil { return nil, fmt.Errorf("chain '%s', step %d, field '%s': failed to compile regex '%s': %w", yamlChain.Name, yamlStep.Order, field, regexStr, err) }
...
func CheckChains(entry *LogEntry) {
	...
	getMatchValue := func(fieldName string, entry *LogEntry) (string, error) {
		switch fieldName {
		case "IP": return entry.IP, nil
		case "Path": return entry.Path, nil
		case "Method": return entry.Method, nil
		case "UserAgent": return entry.UserAgent, nil
		case "Referrer": return entry.Referrer, nil
		case "StatusCode": return strconv.Itoa(entry.StatusCode), nil
		default: return "", fmt.Errorf("unknown field: %s", fieldName)
		}
	}
	...
	for _, chain := range currentChains {
		...
		mutex := &ActivityMutex
		if dryRun { mutex = &dryRunActivityMutex }
...
func parseLogLine(line string) (*LogEntry, error) {
	...
	if len(parts) < 5 { return nil, fmt.Errorf("malformed log line") }
	...
	if len(ipPart) < 2 || len(requestPart) < 2 || len(statusSizePart) < 1 { return nil, fmt.Errorf("malformed essential fields (missing IP, Hostname, or Request)") }
	...
	if err != nil { return nil, fmt.Errorf("failed to parse status code: %w", err) }
